"""
Train gene essentiality classifier on a single cancer type.

Corresponds to the original geneformer_ess.py but uses the clean essgene package
instead of modified geneformer site-packages.

Usage:
    python -m essgene.scripts.train_single --cancer cesc --cell_line caski --only_train
    python -m essgene.scripts.train_single --cancer cesc --cell_line caski --only_test
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train single-cancer essentiality classifier")
...
def main():
    ...
    print(f"Expression features: {len(exp_feat_dict)}, Protein features: {len(unip_feat_dict)}")
    ...
    # Resolve paths
    cl = config["labels"][args.cancer].split("/")[-1].split("_")[0]
    if args.cell_line:
        cl = args.cell_line
    ...
    if not args.only_test:
        ...
        suffix = f"train_{args.layers}l_{cl}_freez_{args.freeze_layers}_{args.suf}"
        if args.exclude_pan:
            suffix = f"train_{args.layers}l_{cl}_freez_wo_pan_{args.freeze_layers}_{args.suf}"
    ...
    if not args.only_train:
        ...
        suffix = f"test_{args.layers}l_{cl}_freez_{args.freeze_layers}_{args.suf}"
        finetuned_base = (
            f"{output_base}/train_{args.layers}l_{cl}_freez_{args.freeze_layers}_{args.suf}"
        ...
        if args.cancer in cancer_summary_map:
            cell = cancer_summary_map[args.cancer]
            for week in ["3w", "4w"]:
                key = f"{cell}_{week}"
        ...
        for ckp_path in checkpoint_list:
            ...
            pred_dict = {
                ...
                "y_logit_pos": real_logits[:, 1],
                "y_logit_neg": real_logits[:, 0],
            ...
            # Hit@n evaluation
            if args.cancer not in ["brca", "coad"] and gene_summaries:
